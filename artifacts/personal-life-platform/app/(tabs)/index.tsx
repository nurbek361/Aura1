...
export default function HomeScreen() {
  ...
  return (
    <Screen>
      ...
      <Pressable onPress={() => router.push('/movies')}><Card style={{ marginTop: 18, backgroundColor: colors.card }}><View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}><IconBadge icon="play-circle" color={colors.violet ?? colors.primary} /><View style={{ flex: 1 }}><Text style={{ color: colors.foreground, fontSize: 16, fontWeight: '700' }}>Aura Cinema</Text><Text style={{ color: colors.mutedForeground, marginTop: 3 }}>Фильмы с постерами и просмотром внутри Aura</Text></View><Feather name="chevron-right" size={18} color={colors.mutedForeground} /></View></Card></Pressable>\n      <SectionTitle title="Фокус дня" action={<Pressable onPress={() => open('/planner')}><Text style={{ color: colors.primary, fontWeight: '700' }}>Открыть</Text></Pressable>} />
